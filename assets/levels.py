...
class TestLevel:
    def __init__(self):
        ...
        self.cache = []
        self.key_hold_time = {}
        self.move_delay = 0.25  # Delay before smooth movement in seconds
        self.next_move_delay = 0.1
        self.last_action_time = time()

    @property
    def snapshot(self):
        return deepcopy(
            [(type(model), model.pos, model.color) for model in self.content]
        )

    def restore(self, snapshot):
        timeless_content = [model for model in self.content if model.is_timeless]
        restored_content = [
            model(pos[0], pos[1], color)
            for model, pos, color in snapshot
            if not model.is_timeless
        ]
            ...
            for s in starters:
                for f in finishers:
                    if f.linked_model is not None:
                        rules_in_a_list.append(
                            self.create_is_rule(s.linked_model, f.linked_model)
                        )
                    if isinstance(f, TextTimeless):
                        rules_in_a_list.append(
                            self.create_is_property(s.linked_model, f)
                        )
    ...
    def get_rules(self):
        ...
        for row in range(GRID_HEIGHT):
            all_rules += self.extract_rules_from_a_list(
                [self.map.get((col, row), []) for col in range(GRID_WIDTH)]
            )
        for col in range(GRID_WIDTH):
            all_rules += self.extract_rules_from_a_list(
                [self.map.get((col, row), []) for row in range(GRID_HEIGHT)]
            )
        return all_rules
    ...
    @cache_method
    def try_move(
        self, model: ModelBase, direction: Tuple[int, int]
    ) -> Tuple[bool, List[ModelBase]]:
    ...
    def perform_action(self):
        ...
        if 0 < current_time - self.key_hold_time[last_pressed_key] < self.move_delay:
            return
        if current_time - self.last_action_time < self.next_move_delay:
            return
